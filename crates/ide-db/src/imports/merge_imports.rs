...
fn recursive_merge(
    ...
    let mut lhs_list = lhs.use_tree_list()?.use_trees().collect::<Vec<_>>();
    let mut use_trees = lhs_list
        .iter()
        .enumerate()
    ...
    use_trees.sort_unstable_by(|&a, &b| use_tree_cmp(&lhs_list[a], &lhs_list[b]));
    for rhs_t in rhs.use_tree_list()?.use_trees() {
        if !merge.is_tree_allowed(&rhs_t) {
            return None;
        }

        match use_trees.binary_search_by(|&idx| use_tree_cmp_bin_search(&lhs_list[idx], &rhs_t)) {
            Ok(idx) => {
                let lhs_idx = use_trees[idx];
                let mut lhs_t = lhs_list[lhs_idx].clone();
            ...
            Err(insert_idx) => {
                ...
                use_trees.insert(insert_idx, lhs_idx);
            }
        }
    }
    with_use_tree_list(&lhs, lhs_list, make)
}
...
fn make_use_tree_list(
    ...
    match style_source.and_then(use_tree_list_style) {
        Some(style) => apply_use_tree_list_style(&use_tree_list, style),
        _ => Some(use_tree_list),
    }
}
...
fn use_tree_list_style(use_tree_list: &ast::UseTreeList) -> Option<(String, String, bool)> {
    ...
    let inner = text.strip_prefix('{').and_then(|text| text.strip_suffix('}')).unwrap_or("");
    let leading_ws = inner.chars().take_while(|c| c.is_whitespace()).collect::<String>();
    let trailing_ws = inner
        .chars()
        .rev()
    ...
    (!leading_ws.is_empty() || !trailing_ws.is_empty() || trailing_comma).then_some((
        leading_ws,
        trailing_ws,
        trailing_comma,
    ))
...
fn apply_use_tree_list_style(
    use_tree_list: &ast::UseTreeList,
    (leading_ws, trailing_ws, trailing_comma): (String, String, bool),
) -> Option<ast::UseTreeList> {
    ...
    if !leading_ws.is_empty() {
        editor
            .insert(Position::after(use_tree_list.l_curly_token()?), make.whitespace(&leading_ws));
    }

    let has_trees = use_tree_list.use_trees().next().is_some();
    let mut trailing = Vec::new();
    if trailing_comma && has_trees {
        trailing.push(make.token(T![,]).into());
    }
    if !trailing_ws.is_empty() {
        trailing.push(make.whitespace(&trailing_ws).into());
    }
    if !trailing.is_empty() {
        editor.insert_all(Position::before(use_tree_list.r_curly_token()?), trailing);
    }
